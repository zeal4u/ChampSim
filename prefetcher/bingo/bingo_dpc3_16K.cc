...
class PatternHistoryTable : public LRUSetAssociativeCache<PatternHistoryTableData> {
  ...
  public:
    ...
    vector<vector<bool>> find(uint64_t pc, uint64_t address) {
        ...
        for (int i = 0; i < (int)matches.size(); i += 1)
            // 这里rotate不就转回去了吗？
            // 这里的rotate很重要, 不改的话会导致性能大幅下降
            // 这个应该就是那篇2011 SMS的论文提到的改进有关
  ...
  private:
    ...
    /* Storage = size * (37 - lg(sets) + 32 + 1 + lg(ways)) */
    /* 16K * (37 - lg(2048) + 32 + 1 + lg(8)) = 124K Bytes    */
    /*======================================================*/
};
    ...
    int prefetch(CACHE *cache, uint64_t block_address) {
        if (this->debug_level >= 2) {
            cerr << "PrefetchStreamer::prefetch(cache=" << cache->NAME << ", block_address=0x" << hex << block_address
                 << ")" << dec << endl;
            cerr << "[PrefetchStreamer::prefetch] " << cache->get_occupancy(3,0) << "/" << cache->get_size(3,0)
                 << " PQ entries occupied." << dec << endl;
            cerr << "[PrefetchStreamer::prefetch] " << cache->get_occupancy(0,0) << "/" << cache->get_size(0,0)
            ...
            for (int sgn = +1; sgn >= -1; sgn -= 2) {
                ...
                if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
                    ...
                    if (cache->get_occupancy(3,0) + cache->get_occupancy(0,0) < cache->get_size(0,0) - 1 &&
                        ...
                        int ok = cache->prefetch_line(pf_address, pattern[pf_offset], 0, true);
                        // assert(ok == 1);
                        pf_issued += 1;
                        pattern[pf_offset] = 0;
                    } else {
...
class Bingo {
  public:
    ...
    void log() {
        ...
        cerr << "Pattern history table begin" << dec << endl;
        cerr << this->pht.log();
        cerr << "Pattern history table end" << endl;

        cerr << "Prefetch streamer begin" << dec << endl;
        cerr << this->pf_streamer.log();
        cerr << "Prefetch streamer end" << endl;
    }
...
void CACHE::prefetcher_initialize() {
    ...
    const int PHT_SIZE = 16 * 1024;     /* size of pattern history table (PHT) */
    const int PHT_WAYS = 8;           /* associativity of PHT */
