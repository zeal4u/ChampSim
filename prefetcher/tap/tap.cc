...
{
    const addr_t val;
    tap_eq_addr(addr_t val) : val(val) {}
    bool operator()(const typename T::value_type &test)
    {
        tap_is_valid<T> validtest;
        return validtest(test) && test.address == val;
    }
};
// 
/**
 * A specialization of tap_eq_addr for descendency lists
 * because they do not have addresses, but (idx, offset) pairs.
 */
template<>
struct tap_eq_addr<descendency_t>
{
    std::size_t idx;
    unsigned int offset;
    bool matchnone;
    tap_eq_addr(const addr_t val)
    {
        addr_t page_addr;
        std::tie(page_addr,offset) = region_offset(val);
        auto it = std::find_if(page_translation_buffer.begin(), page_translation_buffer.end(), tap_eq_addr<page_translation_buffer_t>(page_addr)); // sneakly read the translation buffer
...
bool shadow_cache_filter::check_useful(const addr_t block_addr)
{
    ...
    auto it = std::find_if(set.begin(), set.end(), tap_eq_addr<cache_t::value_type>(tag));
...
std::pair<std::size_t, unsigned int> addr_to_idx(const addr_t block_addr)
{
    ...
    auto it = std::find_if(page_translation_buffer.begin(), page_translation_buffer.end(), tap_eq_addr<page_translation_buffer_t>(page_addr));
    if (it == page_translation_buffer.end())
    {
        it = std::find_if_not(page_translation_buffer.begin(), page_translation_buffer.end(), tap_is_valid<page_translation_buffer_t>());
        if (it == page_translation_buffer.end())
        {
            // Replace NRU
...
std::vector<addr_t> follow_path_dfs(const addr_t block_addr, const double scale_factor, const std::size_t depth)
{
    ...
    bool is_empty = std::none_of(container.begin(), container.end(), tap_is_valid<descendency_t>());
...
std::pair<bool, descendency_t::iterator> find_or_inc(const addr_t block_addr, const uint64_t value, const int inc, const bool replace)
{
    ...
    auto it = std::find_if(container.begin(), container.end(), tap_eq_addr<descendency_t>(value));
