...
uint32_t O3_CPU::prefetcher_cache_operate(uint64_t v_addr, uint8_t cache_hit, uint8_t prefetch_hit, uint32_t metadata_in)
{
  ...
  if (ShadowMiss) {
    ...
    /////
    if ((Block != (PREVADDR[0] >> 4) + 1) || (MAXFNL == 0)) { // Link Block to the address of the block that missed DISTAHEAD+1 before
      ...
      if ((PREFCAND[AHEAD.distahead] != 0) & (PREFCAND[AHEAD.distahead] != Block))
        AHEADphist.LinkAhead(Block, PREVADDR[AHEADphist.distahead] ^ (PREVADDR[AHEADphist.distahead + NSHIFT] << 1), cache_hit);
    }
...
{
  // cout << hex << "fill: 0x" << v_addr << dec << " " << set << " " << way << " " << (uint32_t)prefetch << " " << hex << "evict: 0x" << evicted_v_addr << dec
  // << endl;
  return metadata_in;
}
