...
namespace knob
{
  ...
  uint32_t scooby_pt_address_hash_type = 11;
  // uint32_t scooby_pt_address_hashed_bits = ;
  uint32_t scooby_bloom_filter_size = 2048;
  ...
  vector<float> le_featurewise_feature_weights = {1., 1.};
...
uint32_t Scooby::predict(uint64_t base_address, uint64_t page, uint32_t offset, shared_ptr<State> &state, vector<uint64_t> &pref_addr)
{
 ...
 * false otherwise */
bool Scooby::track(uint64_t address, shared_ptr<State> &state, uint32_t action_index, Scooby_PTEntry **tracker)
{
...
void Scooby::update_stats(shared_ptr<State> &state, uint32_t action_index, uint32_t degree)
{
