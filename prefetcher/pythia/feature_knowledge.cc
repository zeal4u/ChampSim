...
float FeatureKnowledge::retrieveQ(shared_ptr<State> &state, uint32_t action)
{
...
void FeatureKnowledge::updateQ(shared_ptr<State> &state1, uint32_t action1, int32_t reward, shared_ptr<State> &state2, uint32_t action2)
{
...
uint32_t FeatureKnowledge::get_tile_index(uint32_t tiling, shared_ptr<State> &state)
{
...
uint32_t FeatureKnowledge::getMaxAction(shared_ptr<State> &state)
{
...
string FeatureKnowledge::get_feature_string(shared_ptr<State> &state)
{
	uint64_t pc = state->pc;
	// uint64_t page = state->page;
	// uint64_t address = state->address;
...
void FeatureKnowledge::dump_feature_trace(shared_ptr<State> &state)
{
	trace_timestamp++;
