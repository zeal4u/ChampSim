...
private:
	void init_knobs();
	void init_stats();
	uint32_t getMaxAction(shared_ptr<State> &state, float &max_q, float &max_to_avg_q_ratio, vector<bool> &consensus_vec);
	float consultQ(shared_ptr<State> &state, uint32_t action);
	void gather_stats(float max_q, float max_to_avg_q_ratio);
	void action_selection_consensus(shared_ptr<State> &state, uint32_t selected_action, vector<bool> &consensus_vec);
	void adjust_feature_weights(vector<bool> consensus_vec, RewardType reward_type);
	bool do_fallback(shared_ptr<State> &state);
	void plot_scores();
...
public:
	...
	uint32_t chooseAction(shared_ptr<State> &state, float &max_to_avg_q_ratio, vector<bool> &consensus_vec);
	void learn(shared_ptr<State> &state1, uint32_t action1, int32_t reward, shared_ptr<State> &state2, uint32_t action2, vector<bool> consensus_vec, RewardType reward_type);
	void dump_stats();
};
