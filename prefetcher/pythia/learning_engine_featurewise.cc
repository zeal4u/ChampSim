...
uint32_t LearningEngineFeaturewise::chooseAction(shared_ptr<State> &state, float &max_to_avg_q_ratio, vector<bool> &consensus_vec)
{
	stats.action.called++;
	uint32_t action = 0;
...
void LearningEngineFeaturewise::learn(shared_ptr<State> &state1, uint32_t action1, int32_t reward, shared_ptr<State> &state2, uint32_t action2, vector<bool> consensus_vec, RewardType reward_type)
{
	stats.learn.called++;
	if(m_type == LearningType::SARSA && m_policy == Policy::EGreedy)
	{
...
uint32_t LearningEngineFeaturewise::getMaxAction(shared_ptr<State> &state, float &max_q, float &max_to_avg_q_ratio, vector<bool> &consensus_vec)
{
...
float LearningEngineFeaturewise::consultQ(shared_ptr<State> &state, uint32_t action)
{
...
void LearningEngineFeaturewise::action_selection_consensus(shared_ptr<State> &state, uint32_t selected_action, vector<bool> &consensus_vec)
{
	stats.consensus.total++;
...
bool LearningEngineFeaturewise::do_fallback(shared_ptr<State> &state)
{
