...
private:
	...
	uint32_t get_tile_index(uint32_t tiling, shared_ptr<State> &state);
	string get_feature_string(shared_ptr<State> &state);
	...
	/* for q-value tracing */
	void dump_feature_trace(shared_ptr<State> &state);
	void plot_scores();
...
public:
	...
	~FeatureKnowledge();
	float retrieveQ(shared_ptr<State> &state, uint32_t action_index);
	void updateQ(shared_ptr<State> &state1, uint32_t action1, int32_t reward, shared_ptr<State> &state2, uint32_t action2);
	static string getFeatureString(FeatureType type);
	uint32_t getMaxAction(shared_ptr<State> &state); /* Called by featurewise engine only to get a consensus from all the features */
