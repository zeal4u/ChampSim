...
uint64_t g_prefetch_turns=0, g_prefetch_l1=0, g_prefetch_l2=0, g_prefetch_degree=0;
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
    ...
    do {
        ...
        bool prefetch_degree_count = false;
            ...
            if ((addr & ~(PAGE_SIZE - 1)) == (pf_addr & ~(PAGE_SIZE - 1))) { // Prefetch request is in the same physical page
				...
				if ( num_pf < ceil(((get_size(3,0))/distinct_pages)) ) {					
                	if (FILTER.check(pf_addr, train_addr, curr_ip, fill_level, train_delta + delta_q[i], last_sig, curr_sig, confidence_q[i], perc_sum, (depth-1))) {
						...
						prefetch_line(pf_addr, ((fill_level == SPP_L2C_PREFETCH) ? FILL_L1 : FILL_L2), encode_metadata(0, CPLX_TYPE, 0), true); 
                        prefetched = true;
                        prefetch_degree_count = true;
                        int _ = fill_level == SPP_L2C_PREFETCH ? g_prefetch_l1++ : g_prefetch_l2++;
						num_pf++;
...
#ifdef LOOKAHEAD_ON
        g_prefetch_degree += prefetch_degree_count;
    } while (do_lookahead);
#endif
    g_prefetch_turns += prefetched;
    return metadata_in;
}
